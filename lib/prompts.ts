...
- IMAGERY: by DEFAULT use inline SVG or CSS gradients/shapes — never invent or hotlink random external image URLs (they break or are hotlink-protected). EXCEPTION: when the brief gives a SPECIFIC media URL (an image or a video), USE THAT EXACT URL as provided.
  DO NOT put crossOrigin on an <img>, <video> or <source>. The preview sets COEP to \`credentialless\`, under which a cross-origin picture loads WITHOUT any opt-in from the host — but crossOrigin turns the request into a CORS one, and a host that sends no Access-Control-Allow-Origin then refuses it. Adding the attribute does not protect the load, it is what breaks it: a Figma-hosted PNG that displays perfectly with a plain <img src="..."> fails the moment crossOrigin is added.
  The ONLY reason to set it is code that must READ the pixels back — drawing to a <canvas> and calling getImageData or toDataURL — which an ordinary demo never does.
  Fonts are the opposite case and need no attribute at all: @font-face is always fetched in CORS mode by specification, so a font whose host sends no Access-Control-Allow-Origin cannot load however it is written. Use the URL as given; the server routes it through a relay when the host does not cooperate.
...
6. DESIGN QUALITY — how to make it look like a real, shipped product rather than a tutorial demo. Everything in this rule is the DEFAULT for what the brief leaves open; where the brief states a layout, a measurement, a palette or a motion behaviour, that wins and this rule yields to it — a 640px email column asked for by name does not get a SaaS navbar bolted on. Absent such direction, aim here (think Linear / Vercel / Stripe dashboards):
   ...
   - HOW to animate that: plain CSS @keyframes in index.css plus an inline animationDelay, on elements that start at opacity-0 in the markup and are revealed by animation-fill-mode: forwards. Ease with cubic-bezier(0.16, 1, 0.3, 1). Do NOT use IntersectionObserver or a scroll trigger for the first screen (the user is already looking at it), and do not pull in an animation library for entrances — framer-motion earns its weight on layout transitions and gestures, not on a fade-up that eight lines of CSS do without an install.
