...
export const PREMIUM_ADVICE_SYSTEM = `คุณคือที่ปรึกษาที่ช่วยเจ้าของงานตัดสินใจว่า "เวอร์ชัน Premium" ของระบบนี้ควรมีอะไร
...
- เหตุผลต้องพูดถึง "ระบบนี้" โดยเฉพาะ อ้างจากเอกสารหรือหน้าจอที่มีจริง
...
export function buildPremiumAdviceUser(input: {
  ...
  const list = input.options
    ...
    .join("\n");
  return [
    "รายการที่ขายเพิ่มได้สำหรับระบบนี้ (เลือกจากนี้เท่านั้น):",
    list,
    "",
    input.screens.length ? `หน้าจอที่มีในระบบ: ${input.screens.join(", ")}` : "",
    input.orgContext ? `\n${input.orgContext}` : "",
    input.brd ? `\n--- BRD ---\n${input.brd.slice(0, 8_000)}` : "",
    input.prd ? `\n--- PRD ---\n${input.prd.slice(0, 8_000)}` : "",
    "",
    "ประเมินว่าเวอร์ชัน Premium ของระบบนี้ควรมีข้อไหนบ้าง และทำไม",
  ]
    .filter(Boolean)
    .join("\n");
}
